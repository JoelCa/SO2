...
//Agregado para el ejerc. 3 (Plancha 2)
#define NUM_LIST 5
...
//Aclaración: Es un thread al cual NO deberia aplicarsele un join,
//pero puede aplicar join a otro threads
Thread::Thread(const char* threadName)
{
    ...
    threadsJoineables = new List<Thread*>();
    joineable = false;
    puerto = NULL;

    //Agregado para el ejerc. 4 (Plancha 2)
    priority = rand()%NUM_LIST;
    DEBUG('t', "La prioridad del thread %s es: %d\n", name, priority);
    lTPriority = -1;
    timePriorities = new List<int>();
...
//Modificado para el ejerc. 3 (plancha 2) 
Thread::Thread(const char* threadName, bool join)
{
  ...
  if(joineable) {
    puerto = new Puerto();
    parentThread->threadsJoineables->Append(this);
  }
  threadsJoineables = new List<Thread*>();
  name = threadName;
  stackTop = NULL;
...
//Modificado para el ejerc. 3 y 4 (plancha 2) 
Thread::~Thread()
{
    ...
    if (stack != NULL)
      DeallocBoundedArray((char *) stack, StackSize * sizeof(HostMemoryAddress)); //a veces recibo SIGSEGV por esta línea
    delete threadsJoineables;
    delete timePriorities;
    delete opFiles;
}
...
//Cambia temporalmente la prioridad, modificando la var. lTPriority.
//Si append es true, guarda en la lista de prio. ,timePriorities, la prioridad modificada.
void
Thread::SetTimePriority(int p, bool append)
{
  if((lTPriority != -1) && append)
    timePriorities->Prepend(lTPriority);
  lTPriority = p;
}

int
Thread::RemoveTimePriority(void)
{
  if(timePriorities->IsEmpty())
    return -1;
  return timePriorities->Remove();
}
...
//Modificado para ejer. 3 (plancha 2)
void
Thread::Finish ()
{
  Thread *childThread;
  ...
  if(joineable)
    puerto->Send(message);
  //hace join a los threads que se olvido esperar mediante un join
  while(!threadsJoineables->IsEmpty()) {
    childThread = threadsJoineables->Remove();
...
//Agregado para ejer. 3 (plancha 2)
void
Thread::Join()
{
...
//Agregados para el ejercicio 2 (plancha 3)
OpenFile *Thread::findOpFile(int fd, bool flag)
{
  if(flag)
    return opFiles->RemoveItemPrio(fd); 
  return opFiles->ItemPrio(fd);
}

int
Thread::addOpFile(OpenFile *op)
{
  opFiles->SortedInsert(op, fileDescriptor);
  fileDescriptor++;
