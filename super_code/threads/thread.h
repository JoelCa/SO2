...
class Thread {
  private:
    ...
    //Agregados para el ejerc. 3 (plancha 2)
    Puerto *puerto;
    bool joineable;
    List<Thread*> *threadsJoineables;
    Thread* parentThread;
    
    //Agregados para el ejerc. 4 (plancha 2)
    int priority; //la prioridad del thread con la que es creado
    int lTPriority; //la prioridad temporal actual
    List<int> *timePriorities;

    //Agregados para el ejerc. 2 (plancha 3)
    List <OpenFile *> *opFiles;
    int fileDescriptor;
    int message;

    //Agreagados para el ejercicio 2 (plancha 4)
    int accessCounter;
    int faultCounter;
  ...
  public:
    ...
    void SetTimePriority(int p, bool append);
    int GetTimePriority(void) {return lTPriority; };
    int RemoveTimePriority(void); 
    ThreadStatus getStatus() { return status; };

    //Agregados para el ejerc 2 (plancha 3)
    OpenFile* findOpFile(int fd, bool flag);
    int addOpFile(OpenFile *op);
  
    int getMsj() { return message; };
    void setMsj(int m) { message = m; };

    static int pidCounter;
    static List <Thread *> *tList;
...
//Agregado para el ejerc. 2 (plancha 3)
//static List <Thread *> *tList = new List <Thread *> ();
//static int pidCounter;
